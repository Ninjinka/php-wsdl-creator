 ...
 *
 * @author Andreas Muller-Saala, wan24.de
 */
class PhpWsdlZend
{
    /**
     ...
     * @param null $data Hook data (default: NULL)
     * @return Zend_Soap_Server The Zend server object
     * @throws Exception
     */
    public static function CreateServer($server, $data = null): ?Zend_Soap_Server
    {
        if (!is_null(self::$Server))
            return self::$Server;
        ...
        if (!is_null($data)) {
            if (PhpWsdl::CallHook(
                ...
                if (is_object($data['class'])) {
                    self::$Server->setObject($data['class']);
                } else {
                    self::$Server->setClass($data['class']);
                }
            }
        }
        return null;
