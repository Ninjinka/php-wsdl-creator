...
class PhpWsdlServers
{
     ...
     */
    public function __construct(PhpWsdl $server)
    {
        $this->GUID = uniqid();
    ...
    {
        return self::$EnableJson && ((self::$UseServer && self::$UseServer->ForceOutputJsonPhp) || ((self::HasParam('phpjsonclient') || self::HasParam('PHPJSONCLIENT')) && !self::$UseServer->ForceNotOutputJsonPhp));
    }
    ...
    {
        return self::$EnableJson && ((self::$UseServer && self::$UseServer->ForceOutputJsonJs) || ((self::HasParam('jsjsonclient') || self::HasParam('JSJSONCLIENT')) && !self::$UseServer->ForceNotOutputJsonJs));
    }
    ...
    {
        return self::$EnableHttp && ((self::$UseServer && self::$UseServer->ForceOutputHttpPhp) || ((self::HasParam('phphttpclient') || self::HasParam('PHPHTTPCLIENT')) && !self::$UseServer->ForceNotOutputHttpPhp));
    }
    ...
    {
        return self::$EnableRest && ((self::$UseServer && self::$UseServer->ForceOutputRestPhp) || ((self::HasParam('phprestclient') || self::HasParam('PHPRESTCLIENT')) && !self::$UseServer->ForceNotOutputRestPhp));
    }
    ...
    {
        return self::$EnableRpc && ((self::$UseServer && self::$UseServer->ForceOutputRpcPhp) || ((self::HasParam('phprpcclient') || self::HasParam('PHPRPCCLIENT')) && !self::$UseServer->ForceNotOutputRpcPhp));
    }
    ...
    public function HandleJsonRequest()
    {
        ...
        $json = preg_replace('/([^\\\\])\\\\n/', '$1\\n', $json);        $req = json_decode(self::GetParam($json));
        ...
        echo StringUtil::json_encode($temp);
        return false;
    }
    ...
    public static function EncodeType($type, $value, $parameter = false)
    {
        ...
        if (get_class($t) == 'PhpWsdlEnum')
            $type = $t->Type;
        return (in_array($type, PhpWsdl::$BasicTypes)) ? $value : StringUtil::json_encode($value);
    }
        ...
        while (++$i < $mLen) {
            ...
            while (++$j < $rLen) {
                ...
                if (!str_ends_with($call, '/'))
                    $call .= '/';
                if ((!is_null($param) && str_starts_with($path, $call)) || $path == $call) {
    ...
    public static function CreatePhpCallHook($data)
    {
        ...
        switch ($data['options']['serversext']) {
            case 'json':
                ...
                $res[] = "\t\t\t" . '"param"=>$param';
                $res[] = "\t\t" . ');';
                $res[] = "\t\t" . 'return json_decode(file_get_contents(self::$_EndPoint."?JSON=".urlencode(StringUtil::json_encode($call))));';
                break;
            case 'rpc':
    ...
    public static function CreateHtmlGeneralHook($data)
    {
        ...
        if (self::$EnableJson) {
            $temp[] = 'JSON';
            if (!isset($server->ForceNotOutputJsonPhp) || !$server->ForceNotOutputJsonPhp) {
                $url = self::$UseServer->GetJsonPhpUri();
                $res[] = '<p>PHP JSON client download URI: <span class="pre"><a href="' . $url . '">' . $url . '</a></span></p>';
            }
            if (!isset($server->ForceNotOutputJsonJs) || !$server->ForceNotOutputJsonJs) {
        ...
        if (self::$EnableRpc && (!isset($server->ForceNotOutputRpcPhp) || !$server->ForceNotOutputRpcPhp)) {
            $temp[] = 'XML RPC';
            $url = self::$UseServer->GetRpcPhpUri();
            $res[] = '<p>PHP XML RPC client download URI: <span class="pre"><a href="' . $url . '">' . $url . '</a></span></p>';
        }
        if (self::$EnableHttp && (!isset($server->ForceNotOutputHttpPhp) || !$server->ForceNotOutputHttpPhp)) {
            $temp[] = 'http';
            $url = self::$UseServer->GetHttpPhpUri();
            $res[] = '<p>PHP http client download URI: <span class="pre"><a href="' . $url . '">' . $url . '</a></span></p>';
        }
        if (self::$EnableRest && (!isset($server->ForceNotOutputRestPhp) || !$server->ForceNotOutputRestPhp)) {
            $temp[] = 'REST';
    ...
    public function RpcCallHandler($method, $param)
    {
        ...
        if (!self::$NoRpcRecode) {
            ...
            while (++$i < $len) {
                ...
                $req['param'][$i] = $this->RecodeParameter($req['param'][$i], $t, $this->Server);
            }
        }
        if (PhpWsdl::$Debugging)
            PhpWsdl::Debug('Parameters: ' . print_r($req['param'], true));
